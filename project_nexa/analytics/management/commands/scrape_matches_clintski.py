...
class Command(BaseCommand):
    ...
    def handle(self, *args, **kwargs):
        ...
        URL = "https://wank.wavu.wiki/player/4i554h5BmTMJ?char=bryan&limit=5000"
        ...
        match_table_container = soup.find("div", class_="game-list")
        if not match_table_container:
            self.stdout.write(self.style.ERROR("Match container not found!"))
            return

        match_table = match_table_container.find("table")
        if not match_table:
            self.stdout.write(self.style.ERROR("Match table not found inside container!"))
            return
